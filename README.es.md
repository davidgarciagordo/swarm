...
4. **Diseño (construido).** `design-orchestrator` + `pattern-advisor`, `domain-modeler`, `planner`; corre solo en `tier: full`, tras discovery cerrar decisiones de producto o directamente desde un objetivo de refactor/migración que se saltó discovery pero aún necesita un rediseño; grill×3 (`working-methods:grill-architect/operator/engineer`) revisa adversarialmente el plan que escribe `planner`, y `design-orchestrator` arbitra los hallazgos él mismo — sin `AskUserQuestion` de por medio.
